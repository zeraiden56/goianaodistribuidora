...
int interaction(float px,float pz,float yaw,bool expanded,bool secondCheckout) {
...
bool walkable(float x,float z,bool expanded) {
    if(!std::isfinite(x)||!std::isfinite(z)||x<=-4.65f||x>=4.65f||z<=-1.78f)return false;
    if(!expanded)return z<2.5f&&!(x>2.48f&&z>1.f)&&!(x< -2.65f&&z>-.6f&&z<1.4f);
    if(z>=9.65f)return false;
    if(x>2.48f&&z>1.f&&z<2.6f)return false; // Computer desk.
    ...
    if(x>2.95f&&z>4.8f&&z<7.65f)return false; // Sofa.
...
float movePlayer(Player& p,float forward,float strafe,bool sprint,float intoxication,float dt,bool expanded) {
    ...
    float x=p.x,z=p.z;
    if(walkable(p.x+dx,p.z,expanded))p.x+=dx;
    if(walkable(p.x,p.z+dz,expanded))p.z+=dz;
