...
struct ComputerView {float x,y,z,yaw,pitch,fov;};
ComputerView computerView(const Player& player,int terminal,float focus,float fov,float aspect);
void renderTerminal(const Game& game,int terminal);
