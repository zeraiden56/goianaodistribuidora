...
namespace {
constexpr int columns=37,rows=17,nodes=columns*rows;
HelperPoint point(int index){return {-4.5f+(index%columns)*.25f,-1.5f+(index/columns)*.25f};}
bool clearSegment(HelperPoint from,HelperPoint to) {
    int steps=std::max(1,int(std::hypot(to.x-from.x,to.z-from.z)/.06f)+1);
    for(int i=0;i<=steps;++i){float f=float(i)/steps;if(!walkable(from.x+(to.x-from.x)*f,from.z+(to.z-from.z)*f))return false;}
    return true;
}
int nearest(HelperPoint position) {
    ...
    for(int i=0;i<nodes;++i) {
        ...
        if(d<distance&&walkable(p.x,p.z)&&clearSegment(position,p)){selected=i;distance=d;}
    }
    return selected;
}
void plan(HelperState& helper,HelperPoint destination) {
    int start=nearest({helper.x,helper.z}),goal=nearest(destination);if(start<0||goal<0)return;
    ...
    while(!queue.empty()&&parent[goal]<0) {
        ...
        for(int delta:{-1,1,-columns,columns}) {
            ...
            auto p=point(next);if(!walkable(p.x,p.z)||!clearSegment(point(current),p))continue;
...
bool travel(HelperState& helper,HelperPoint goal,float dt) {
    if(std::hypot(helper.x-goal.x,helper.z-goal.z)<.025f)return true;
    if(helper.route.empty())plan(helper,goal);
    float remaining=std::clamp(dt,0.f,.1f)*1.8f;
    while(remaining>0&&!helper.route.empty()) {
        ...
        if(!walkable(x,z)){helper.route.clear();return false;}
...
void tickHelper(Game& g,float dt,int lane) {
    auto& h=lane?g.secondHelper:g.helper;
    const auto& customer=lane?g.second.customer:g.customer;
    const auto& wanted=lane?g.second.wanted:g.wanted;
    const auto& quantity=lane?g.second.quantity:g.quantity;
    const auto& delivered=lane?g.second.delivered:g.delivered;if(!h.hired)return;
    ...
    if(h.task==HelperTask::Fetching) {
        if(!travel(h,shelf(h.target),dt))return;
        if(g.products[h.target].stock>0) {
            h.count=std::min({g.bagCapacity,quantity-delivered,g.products[h.target].stock});g.products[h.target].stock-=h.count;g.touchStorage(h.target);h.held=h.target;task(h,HelperTask::Delivering,h.target);h.wait=.35f;
        } else task(h,HelperTask::Idle,-1);
    } else if(h.task==HelperTask::Delivering) {
        if(!travel(h,{lane?2.1f:-2.1f,-1.45f},dt))return;
        while(h.count>0&&customer&&wanted==h.held) {
            int unit=h.held;if(!g.deliver(unit,lane))break;--h.count;
        }
        if(h.count==0){h.held=-1;task(h,HelperTask::Idle,-1);}
        else task(h,HelperTask::Returning,h.held);
        h.wait=.4f;
    } else if(h.task==HelperTask::Returning) {
        if(!travel(h,shelf(h.held),dt))return;
        g.products[h.held].stock+=h.count;h.count=0;g.touchStorage(h.held);h.held=-1;task(h,HelperTask::Idle,-1);h.wait=.2f;
    }
}
